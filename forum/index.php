<?php
/**
 * Forum Home — Categories, Boards, Stats, Search
 */
$pageTitle = 'Forum';
...
$boardRows = $pdo->query(
    'SELECT b.*,
            lp.created_at AS lp_date, lp.user_id AS lp_user_id,
...
/* Forum-wide stats (cached or quick counts) */
      ...
      <table style="margin:0;">
        ...
        <tbody>
          ...
          <?php else: foreach ($catBoards as $b): ?>
            <tr>
              <td>
                <a href="/forum/board.php?id=<?= $b['board_id'] ?>" style="font-weight:600;font-size:.95rem;">
                  📁 <?= e($b['name']) ?>
                </a>
              ...
              <td style="font-size:.82rem;">
                <?php if ($b['lp_date']): ?>
                  ...
                  <div style="color:var(--text-secondary);margin-top:2px;">
                    by <a href="/forum/profile.php?id=<?= $b['lp_user_id'] ?>" style="color:var(--accent-purple);"><?= e($b['lp_username']) ?></a>
      ...
      <a href="/forum/board.php?id=<?= $b['board_id'] ?>" style="display:block;padding:.65rem .5rem;border-bottom:1px solid var(--border-color);color:var(--text-primary);">
        <div style="display:flex;justify-content:space-between;align-items:center;">
          ...
          <span style="font-size:.72rem;color:var(--text-secondary);">
            <?= $b['thread_count'] ?>t / <?= $b['post_count'] ?>p
          </span>
        </div>
        <?php if ($b['description']): ?>
  ...
  <?php foreach ($recentThreads as $rt): ?>
    <div style="padding:.5rem 0;border-bottom:1px solid var(--border-color);display:flex;justify-content:space-between;align-items:center;gap:.5rem;flex-wrap:wrap;">
      <div style="min-width:0;flex:1;">
        <a href="/forum/thread.php?id=<?= $rt['thread_id'] ?>" style="font-weight:600;font-size:.92rem;">
          <?= e($rt['title']) ?>
        </a>
        <div style="font-size:.75rem;color:var(--text-secondary);margin-top:.1rem;">
          ...
          by <a href="/forum/profile.php?id=0" style="color:var(--accent-purple);"><?= e($rt['username']) ?></a>
        </div>
      </div>
      <div style="text-align:right;flex-shrink:0;">
        ...
        <div style="font-size:.72rem;color:var(--text-secondary);">
          <?= $rt['reply_count'] ?> replies &middot; <?= $rt['views'] ?> views
        </div>
      </div>
    </div>
  <?php endforeach; ?>
